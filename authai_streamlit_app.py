...
try:
    from authai_core_cloud import load_best_model_and_meta, RealTimeMonitor, BotSimulator
    CLOUD_MODE = True
except ImportError:
    try:
        ...
        st.error("Could not import authai_core module. Make sure authai_core.py or authai_core_cloud.py is in the same directory.")
        st.stop()
...
def main():
    ...
    # Bot simulation
    st.sidebar.markdown("---")
    st.sidebar.subheader("Bot Simulation")
    
    if st.sidebar.button("🤖 Run Bot Simulator", key="bot_btn", disabled=st.session_state.bot_running):
        if st.session_state.monitor and st.session_state.monitor_running:
            if run_bot_simulation():
                st.sidebar.warning("Bot simulator running for 15 seconds...")
        else:
            st.sidebar.error("Start monitor first!")
    
    if st.session_state.bot_running:
        st.sidebar.warning("🤖 Bot simulation in progress...")
    ...
    with status_col2:
        if st.session_state.bot_running:
            st.markdown('<div class="alert-container"><h4>🤖 Bot Simulation: RUNNING</h4></div>', 
                       unsafe_allow_html=True)
        else:
